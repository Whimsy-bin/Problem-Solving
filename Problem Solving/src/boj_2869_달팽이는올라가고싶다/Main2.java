...
public class Main2 {
	public static void main(String[] args) throws IOException {
		...
		String s = br.readLine(); // Int 정보를 일단 String으로 받기
		StringTokenizer st = new StringTokenizer(s); // StringTokenizer 인자값에 입력받은 데이터를 넣어줌
		int A = Integer.parseInt(st.nextToken()); // 낮에 올라가는 거리 A
		int B = Integer.parseInt(st.nextToken()); // 밤에 미끄러지는 거리 B
		int V = Integer.parseInt(st.nextToken()); // 올라가야할 나무 막대의 높이 V
