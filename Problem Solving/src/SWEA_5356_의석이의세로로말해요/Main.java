package SWEA_5356_의석이의세로로말해요; // 2022.08.17
...
//	칠판에 붙여진 단어들이 주어질 때, 의석이가 세로로 읽은 순서대로 글자들을 출력하는 프로그램을 작성하라.
	...
	public static void main(String[] args) throws IOException {
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			// 각 테스트 케이스마다 문자열이 5개씩 주어지므로 [5][]크기의 2차원 배열을 만들어 입력값을 저장해준다.
			String wordBoard[][] = new String[5][];
			for (int i = 0; i < 5; i++) {
				String temp = br.readLine();
				String arr[] = temp.split("");
				wordBoard[i] = new String[arr.length];
				for (int j = 0; j < arr.length; j++) {
					wordBoard[i][j] = arr[j];
				}
			}
			// 오류까지 고려하여 야무지게 출력만 해주면 끝!
			for (int k = 0; k < 15; k++) {
				for (int l = 0; l < 5; l++) {
					try {
