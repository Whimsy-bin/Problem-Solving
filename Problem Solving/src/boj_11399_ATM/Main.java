package boj_11399_ATM; // 2022.09.05
...
//	줄을 서 있는 사람의 수 N과 각 사람이 돈을 인출하는데 걸리는 시간 Pi가 주어졌을 때, 각 사람이 돈을 인출하는데 필요한 시간의 합의 최솟값을 구하는 프로그램을 작성하시오.
	...
	public static void main(String[] args) throws IOException {
		...
		StringTokenizer st = new StringTokenizer(br.readLine()); // 해당 사람들이 돈을 인출하는 데에 걸리는 시간 정보

		int[] time = new int[N]; //
		for (int i = 0; i < N; i++)
			time[i] = Integer.parseInt(st.nextToken());

		// 인출하는 데에 걸리는 시간이 짧은 사람이 앞에 있을 수록 유리하다. 정렬 순서를 바꾸어주자.
		Arrays.sort(time);

		int ans = 0;
		for (int multiply = N, j = 0; j < N; multiply--, j++)
			ans += multiply * time[j];
